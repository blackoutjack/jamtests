...
function midoriInlineEdit(vars$$8) {
  ...
  this.edit = function() {
    ...
    var value$$29 = this.myObj.innerHTML.replace(/"/g, "&quot;");
    this.myObj.innerHTML = vars$$8.textArea ? midori.replace('<textarea style="width: :w; height: :h; overflow: auto">:value</textarea>', {w:this.myObj.parentNode.offsetWidth - (vars$$8.x ? vars$$8.x : 32) + "px", h:this.myObj.parentNode.offsetHeight - (vars$$8.y ? vars$$8.y : 32) + "px", value:value$$29}) : midori.replace('<input type="text" size=":size" maxlength=":maxlen" value=":value" />', {size:vars$$8.size ? vars$$8.size : "", maxlen:vars$$8.maxlen ? vars$$8.maxlen : "", value:value$$29});
...
var midori = {browserType:window.opera ? "Opera" : navigator.userAgent.indexOf("WebKit") != -1 ? "Safari" : navigator.userAgent.indexOf("MSIE") != -1 ? "MSIE" : "Gecko", browserOS:navigator.userAgent.indexOf("Windows") != -1 ? "Win" : navigator.userAgent.indexOf("Macintosh") != -1 ? "Mac" : "Other", domReady:[], each:function(parentObj, callBack, depthFirst) {
...
}, getMatches:function(target$$26, s$$2, a$$1, oneLevelOnly) {
  this.pCache = {};
  var matches = [];
  var exprs$$1 = [];
  var chunks;
  ...
  if (s$$2.indexOf("#") != -1) {
    ...
    if (s$$2.indexOf(".") != -1) {
      ...
      objs = target$$26.getElementsByTagName(chunks[0] ? chunks[0] : "*");
      i$$3 = 0, numObjs = objs.length;
      for (;i$$3 < numObjs;i$$3++) {
        if (!oneLevelOnly && className.test(objs[i$$3].className) || oneLevelOnly && className.test(objs[i$$3].className) && objs[i$$3].parentNode == target$$26) {
          this.postProcess(objs[i$$3]);
        }
      }
    } else {
      if (s$$2 == "*" || /^[A-Za-z0-9]+$/.test(s$$2)) {
        i$$3 = 0, objs = target$$26.getElementsByTagName(s$$2), numObjs = objs.length;
        for (;i$$3 < numObjs;i$$3++) {
          if (!oneLevelOnly || oneLevelOnly && objs[i$$3].parentNode == target$$26) {
            this.postProcess(objs[i$$3]);
          }
        }
      }
  ...
  return allObjs;
}, getCssRule:function(stylesheet, rule$$2, property) {
  var values$$4 = [];
    ...
    if (rule$$2 == "*" || rules[i$$6].selectorText.toLowerCase() == rule$$2) {
      ...
      for (j$$1 in rules[i$$6].style) {
        if (this.browserType == "Safari") {
          if (rules[i$$6].style[j$$1].toLowerCase && rules[i$$6].style[j$$1].toLowerCase() == property) {
            if (rule$$2 == "*") {
...
}, addEventListener:function(target$$27, eventType$$2, listenerFunc) {
  ...
  target$$27.attachEvent("on" + eventType$$2, listenerFunc);
  ...
  var target$$28 = event$$1.target ? event$$1.target : event$$1.srcElement;
...
}, getSelection:function(target$$29) {
...
}, setCookie:function(name$$30, value$$28, expires, path, domain) {
  setCookieSentinel = true;
  var today = new Date;
  document.cookie = name$$30 + "=" + escape(value$$28) + "; expires=" + today.toUTCString(today.setSeconds(expires)) + (path ? "; path=" + path : "") + (domain ? "; domain=" + domain : "");
...
}, saveCheckboxState:function(element$$2, cb, callback$$25) {
  cb.checked ? element$$2.innerHTML++ : --element$$2.innerHTML == 0 ? element$$2.innerHTML = "" : {};
  if (callback$$25) {
    callback$$25(element$$2.innerHTML);
  }
...
var midoriFX = {intervals:{}, lastIntervalId:0, getOutsideSize:function(element$$3, what) {
  var offsetSize = what == "width" ? element$$3.offsetWidth : element$$3.offsetHeight;
  element$$3.style[what] = offsetSize.toString() + "px";
  var outsideSize = (what == "width" ? element$$3.offsetWidth : element$$3.offsetHeight) - offsetSize;
  element$$3.style[what] = (offsetSize - outsideSize).toString() + "px";
  return outsideSize;
  ...
  var element$$4 = midori.get("#" + id);
  var vars$$2 = {intervalKey:Math.random(), element:element$$4, callback:callback$$26, what:what$$1};
  midori.setStyles(element$$4, {overflow:"hidden", visibility:"hidden", display:"block"});
  vars$$2.elementSize = (horiz ? element$$4.offsetWidth : element$$4.offsetHeight) - this.getOutsideSize(element$$4, what$$1), midori.setStyles(element$$4, {display:"none", visibility:"visible"});
  ...
  var element$$5 = midori.get("#" + id$$1);
  var outsideSize$$1 = this.getOutsideSize(element$$5, what$$2);
  var vars$$4 = {intervalKey:Math.random(), element:element$$5, outsideSize:outsideSize$$1, elementSize:(horiz$$1 ? element$$5.offsetWidth : element$$5.offsetHeight) - outsideSize$$1, callback:callback$$27, constantSpeed:constantSpeed, what:what$$2};
