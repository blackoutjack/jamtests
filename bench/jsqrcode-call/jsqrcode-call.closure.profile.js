...
function Version(versionNumber, alignmentPatternCenters, ecBlocks1$$1, ecBlocks2$$1, ecBlocks3, ecBlocks4) {
  ...
  this.buildFunctionPattern = function() {
    ...
    for(;x$$50 < max$$1;x$$50++) {
      ...
      var y$$33 = 0;
      for(;y$$33 < max$$1;y$$33++) {
        if(x$$50 == 0 && (y$$33 == 0 || y$$33 == max$$1 - 1) || x$$50 == max$$1 - 1 && y$$33 == 0) {
          continue
        }
        bitMatrix.setRegion(this.alignmentPatternCenters[y$$33] - 2, i$$3, 5, 5)
      }
    }
...
function Detector(image$$5) {
  ...
  this.sizeOfBlackWhiteBlackRun = function(fromX, fromY, toX, toY) {
    ...
    var x$$53 = fromX;
    var y$$36 = fromY;
    for(;x$$53 != toX;x$$53 += xstep) {
      var realX = steep ? y$$36 : x$$53;
      var realY = steep ? x$$53 : y$$36;
      if(state == 1) {
...
function BitMatrix(width$$10, height$$9) {
  ...
  this.setRegion = function(left$$2, top$$2, width$$11, height$$10) {
    if(top$$2 < 0 || left$$2 < 0) {
    ...
    if(bottom$$1 > this.height || right$$2 > this.width) {
      throw"The region must fit inside the matrix";
    }
    var y$$40 = top$$2;
    for(;y$$40 < bottom$$1;y$$40++) {
      var offset$$16 = y$$40 * this.rowSize;
      var x$$57 = left$$2;
      for(;x$$57 < right$$2;x$$57++) {
        this.bits[offset$$16 + (x$$57 >> 5)] |= 1 << (x$$57 & 31)
      }
    }
  }
}
...
function BitMatrixParser(bitMatrix$$1) {
  ...
  this.readCodewords = function() {
    ...
    for(;j$$4 > 0;j$$4 -= 2) {
      ...
      var count$$7 = 0;
      for(;count$$7 < dimension$$11;count$$7++) {
        var i$$15 = readingUp ? dimension$$11 - 1 - count$$7 : count$$7;
        var col = 0;
        for(;col < 2;col++) {
...
function QRCodeDataBlockReader(blocks, version$$8, numErrorCorrectionCode) {
  ...
  this.__defineGetter__("DataByte", function() {
    ...
    do {
      ...
      if(mode$$8 != MODE_NUMBER && mode$$8 != MODE_ROMAN_AND_NUMBER && mode$$8 != MODE_8BIT_BYTE && mode$$8 != MODE_KANJI) {
...
GridSampler.checkAndNudgePoints = function(image$$2, points) {
  ...
  var nudged = true;
  var offset$$12 = 0;
  for(;offset$$12 < points.Length && nudged;offset$$12 += 2) {
    var x$$48 = Math.floor(points[offset$$12]);
    var y$$31 = Math.floor(points[offset$$12 + 1]);
    if(x$$48 < -1 || x$$48 > width$$9 || y$$31 < -1 || y$$31 > height$$8) {
  ...
  for(;offset$$12 >= 0 && nudged;offset$$12 -= 2) {
    x$$48 = Math.floor(points[offset$$12]);
    y$$31 = Math.floor(points[offset$$12 + 1]);
    if(x$$48 < -1 || x$$48 > width$$9 || y$$31 < -1 || y$$31 > height$$8) {
      throw"Error.checkAndNudgePoints ";
    }
    nudged = false;
...
GridSampler.sampleGrid3 = function(image$$3, dimension, transform) {
  ...
  for(;y$$32 < dimension;y$$32++) {
    ...
    try {
      x$$49 = 0;
      for(;x$$49 < max;x$$49 += 2) {
        var xpoint = Math.floor(points$$1[x$$49]) * 4 + Math.floor(points$$1[x$$49 + 1]) * qrcode.width * 4;
        var bit = image$$3[Math.floor(points$$1[x$$49]) + qrcode.width * Math.floor(points$$1[x$$49 + 1])];
        qrcode.imagedata.data[xpoint] = bit ? 255 : 0;
        qrcode.imagedata.data[xpoint + 1] = bit ? 255 : 0;
        qrcode.imagedata.data[xpoint + 2] = 0;
        qrcode.imagedata.data[xpoint + 3] = 255;
        if(bit) {
          bits.set_Renamed(x$$49 >> 1, y$$32)
        }
      }
    }catch(aioobe) {
      throw"Error.checkAndNudgePoints";
...
qrcode.process = function(ctx) {
  ...
  if(qrcode.debug) {
    var y$$41 = 0;
    for(;y$$41 < qrcode.height;y$$41++) {
      var x$$59 = 0;
      for(;x$$59 < qrcode.width;x$$59++) {
        var point = x$$59 * 4 + y$$41 * qrcode.width * 4;
        qrcode.imagedata.data[point] = image$$8[x$$59 + y$$41 * qrcode.width] ? 0 : 0;
        qrcode.imagedata.data[point + 1] = image$$8[x$$59 + y$$41 * qrcode.width] ? 0 : 0;
        qrcode.imagedata.data[point + 2] = image$$8[x$$59 + y$$41 * qrcode.width] ? 255 : 0
      }
    }
...
qrcode.getPixel = function(x$$60, y$$42) {
...
qrcode.binarize = function(th) {
  ...
  for(;y$$43 < qrcode.height;y$$43++) {
    ...
    for(;x$$61 < qrcode.width;x$$61++) {
      ...
      ret[x$$61 + y$$43 * qrcode.width] = gray <= th ? true : false
    }
  }
  return ret
};
...
qrcode.grayscale = function() {
  ...
  var y$$44 = 0;
  for(;y$$44 < qrcode.height;y$$44++) {
    var x$$62 = 0;
    for(;x$$62 < qrcode.width;x$$62++) {
