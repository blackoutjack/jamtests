var policy = function() {
  ...
  function pFull(tx) {
    ...
    for(var i = 0;i < len;i++) {
      ...
      if(!states[44] && node.type === "call" && JAM.identical(node.value, _HTMLDocument_prototype_write) && node.argc > 0 && !JAM.identical(node.args[0], undefined)) {
        states[44] = true
      }
      ...
      if(!states[45] && node.type === "call" && JAM.identical(node.value, _HTMLFormElement_prototype_setAttribute) && node.argc > 0 && JAM.identical(node.args[0], "action")) {
        states[45] = true
      }
  ...
  function p107(tx) {
    ...
    for(var i = 0;i < len && (!states[3] || !states[4] || !states[42] || !states[43] || !states[44] || !states[45] || !states[46]);i++) {
      ...
      if(!states[44] && JAM.identical(node.value, _HTMLDocument_prototype_write) && node.argc > 0 && !JAM.identical(node.args[0], undefined)) {
        states[44] = true
      }
  ...
  function p112(tx) {
    var as = tx.getReadSequence();
    var len = as.length;
    for(var i = 0;i < len && (!states[47] || !states[48]);i++) {
      var node = as[i];
      if(!states[47] && node.id === "setAttribute" && true && JAM.identical(node.id, "src")) {
        states[47] = true
      }
  ...
  p112.subsumedBy = pFull;
  p112.itype = "read";
  Object.freeze(p112);
  return{p102:p102, p115:p115, p116:p116, p107:p107, p104:p104, p106:p106, p103:p103, p105:p105, p54:p54, p11:p11, p112:p112, pFull:pFull, woven:true}
}()