...
function SNote(serverPath, divParent) {
  ...
  function handleEditDialogOk() {
    ...
    if (strNewPass.length == 0) {
      ...
      if (strNewPass != strNewPassRetype) {
        ...
        encryptAndSave(readInputNote(), strNewPass, function(strId$$1) {
          if (strId$$1 != "") {
  ...
  function encryptText(strPlainText$$1, strPasswd) {
    if (strPasswd) {
      wordArrPlainText = CryptoJS.enc.Utf8.parse(strPlainText$$1);
      return CryptoJS.AES.encrypt(wordArrPlainText, strPasswd).toString();
    } else {
      return undefined;
    }
  }
  function decryptText(strCipherText$$1, strPasswd$$1) {
    if (strPasswd$$1 != undefined) {
      wordArrPlainText = CryptoJS.AES.decrypt(strCipherText$$1, strPasswd$$1);
