var policy = function() {
  ...
  function pFull(tx) {
    ...
    for (var i = 0;i < len;i++) {
      ...
      if (!states[44] && node.type === "invoke" && (JAM.identical(node.value, _HTMLDocument_prototype_write) && (node.argc > 0 && !JAM.identical(node.args[0], _#undefined)))) {
        states[44] = true;
      }
  ...
  function p111(tx) {
    ...
    for (var i = 0;i < len;i++) {
      ...
      if (true && JAM.instanceof(node.obj, _CSSStyleDeclaration) && !JAM.identical(node.id, "display") || node.id === "textContent" && JAM.identical(node.obj["className"], "destructive-read read-only") || node.id === "src" && JAM.instanceof(node.obj, _HTMLElement) && !JAM.identical(node.value, "plaintext-test.png")) {
        commit = false;
        break;
      }
  ...
  function p102(tx) {
    ...
    for (var i = 0;i < len && (!states[3] || !states[4] || !states[42] || !states[43] || !states[44] || !states[45] || !states[46]);i++) {
      ...
      if (!states[44] && (JAM.identical(node.value, _HTMLDocument_prototype_write) && (node.argc > 0 && !JAM.identical(node.args[0], _#undefined)))) {
        states[44] = true;
      }
  ...
  p13.itype = "write";
  Object.freeze(p13);
  return{p100:p100, p110:p110, p103:p103, p111:p111, p101:p101, p99:p99, p102:p102, p104:p104, p55:p55, p13:p13, pFull:pFull, woven:true};
}()