...
var cm_url = {'51':{url:'http://www.linezing.com/', url_type:'0'},'52':{url:'http://tongji.linezing.com/mystat.html', url_type:'0'}};
		for(var i in cm_url) {
			if(cm_url[i]['url_type']==0 && cm_url[i]['url']==cur_url) {
				url_id = i;
				break;
			}else if( cm_url[i]['url_type']==1 && cm_url[i]['url']==cur_url.substr(0, cm_url[i]['url'].length) ) {
				if( cm_url[i]['url']==cur_url || cur_url.substr(cm_url[i]['url'].length, 1) == '?' || cur_url.substr(cm_url[i]['url'].length, 1) == '#' ) {
					url_id = i;
					break;
				}
			}
